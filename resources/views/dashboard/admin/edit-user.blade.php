...
@section('content')
    <form method="POST" action="{{ url("/user-list/$selectedUser->id") }}" class="mb-6 flex justify-between items-center">
        @csrf
        @method('DELETE')
        <a href="/user-list"
    ...
    <form method="POST" action="{{ url("/user-list/$selectedUser->id") }}">
        @csrf
        @method('PUT')
        <div class="mb-6 flex justify-between items-center gap-3">
            <div class="w-10/12">
                ...
                <input type="text" id="name" name="name"
                    ...
                    placeholder="John Doe" required value="{{ $selectedUser->name }}">
            </div>
            ...
            <div class="w-2/12">
                ...
                <select id="role" name="role"
                    ...
                    <option value="{{ $selectedUser->role }}" selected>{{ ucwords($selectedUser->role) }}</option>

                    <?php $roles = ['admin', 'apoteker', 'dokter', 'pasien']; ?>
        ...
        @if ($selectedUser->role == 'dokter')
            <div class="mb-6">
                ...
                <input type="text" id="specialist" name="specialist"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                    placeholder="Umum" required value="{{ $selectedUser->specialist }}">
            </div>
        @endif
        ...
        <div class="mb-6">
            ...
            <input type="email" id="email" name="email"
                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                placeholder="name@flowbite.com" required value="{{ $selectedUser->email }}">
        </div>
